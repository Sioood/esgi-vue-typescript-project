...
function fullRemovedText(item: Product) {
  return getRemovedIngredients(item).map(r => `sans ${r}`).join(', ')
}

function getBaseIngredients(item: Product) {
  const baseProduct = productStore.products.find(p => p.id === item.id)
...
function getExtraIngredients(item: Product) {
  ...
  const baseCount = base.reduce((acc, ing) => {
    acc[ing] = (acc[ing] || 0) + 1
    return acc
  }, {} as Record<string, number>)

  const currentCount = item.ingredients.reduce((acc, ing) => {
    acc[ing] = (acc[ing] || 0) + 1
    return acc
  }, {} as Record<string, number>)
...
function getExtraIngredientCount(item: Product, ingredientName: string) {
  const baseProduct = productStore.products.find(p => p.id === item.id)
  const baseCount = baseProduct ? baseProduct.ingredients.filter(i => i === ingredientName).length : 0
        ...
        >
          <div>
            ...
            <div class="font-bold text-lg">
              {{ item.name }}
              <span v-if="getRemovedIngredients(item).length > 0 || getExtraIngredients(item).length > 0" class="ml-2 text-orange-500 text-sm">
      (Personnalisé)
    </span>
            </div>
            ...
            <div class="text-sm flex flex-wrap items-center gap-1 mb-1">
              ...
              <template v-else>
                <template v-for="removed in getRemovedIngredients(item)" :key="'removed-' + removed">
